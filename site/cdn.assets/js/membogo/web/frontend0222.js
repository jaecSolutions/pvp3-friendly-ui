...
memboGo.Web.Consent = {
    ...
    init: function (enable, translate, lang) {
        var self = this;
        this.prepareSiteConsent(translate, lang);
        if (enable) {
            var s = document.createElement('script');
            s.src = 'https://cdn.kiprotect.com/klaro/v0.7.18/klaro.js';
            s.defer = true;
            s.setAttribute('data-klaro-config', 'siteConsent');
            ...
            self.rgpd();
            $.map(window.siteConsent.services, function (item) {
    ...
    gtmTrack: function (name) {
        memboGo.core.trackEvent('Site-' + name + '-Accepted');
    },

    prepareSiteConsent: function (translate, lang) {
        window.siteConsent = {
            lang: lang,
            translations: {},
            version: 1,
            elementID: 'site-consent-box',
            styling: {
        ...
        window.siteConsent.translations[lang] = translate;
    },
